...
then
	...
	sshpass -p "Carnival@123" ssh root@$env "bash -s" -- < for server in app01 app02; do ssh $server '$workspace/deploy_on_server.sh -d "$release" "$component" "$abort_on_fail"'; done> $workspace/logs/"${logfile}"
