	...
	then
		...
		do
			...
			then
				...
				sshpass -p "not4dev!" ssh  -o "StrictHostKeyChecking=no" -v root@$ipaddr 'if [ ! -d /root/Releases ]; then mkdir -p /root/Releases; else for folder in `ls /root/Releases`; do if [ `echo ${folder} | grep "_" | wc -l` -eq 0 ]; then mv /root/Releases/${folder} /root/Releases/${folder}_`date +%Y_%m_%d__%H_%M_%S`; fi; done; fi'
				sshpass -p "not4dev!" scp  -o "StrictHostKeyChecking=no" -v -r $workspace/Releases/$release $workspace/tmp root@$ipaddr:/root/Releases
            fi
		done
	else
