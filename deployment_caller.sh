...
then
	...
	sshpass -p "Carnival@123" ssh -A -t root@$env ssh -A -t app02 'if [ ! -d /root/Releases ]; then mkdir -p /root/Releases; else for folder in `ls /root/Releases`; do rm -rf /root/Releases/$folder; done; fi'
	...
	sshpass -p "Carnival@123" ssh -A -t root@$env ssh -A -t app02 'chmod +x /root/bin/deploy_on_server.sh && /root/bin/deploy_on_server.sh -d "$release" "$component" "$abort_on_fail" "app01"'
...
then
	sshpass -p "Carnival@123" scp -r $workspace/deploy_on_server.sh root@$env:/root/bin
	sshpass -p "Carnival@123" 'ssh root@$env chmod +x /root/bin/deploy_on_server.sh && /root/bin/deploy_on_server.sh -r "$release" "$component" "$abort_on_fail" "app01"'
