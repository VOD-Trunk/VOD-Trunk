...
then
    mkdir -p $workspace/logs/Deployment
fi

if [ -f $workspace/logs/Deployment/email_body.txt ]
then
    rm -f $workspace/logs/Deployment/email_body.txt
fi
...
{ #try
    ...
    then
        ...
        then
            ...
            then
                ...
                log "Log file not present at $workspace/logs/Deployment/${logfile}"
                exit 1
            fi
        else
            ...
            #sshpass -p "not4dev!" ssh -o "StrictHostKeyChecking=no" root@$env "bash -s" -- < $workspace/deploy_on_server.sh -d "$release" "$component" "$abort_on_fail" "app01" "$transfer_flag" > $workspace/logs/Deployment/"${logfile}"
            #sshpass -p "not4dev!" ssh -o "StrictHostKeyChecking=no" root@$env 'ssh app02' "bash -s" -- < $workspace/deploy_on_server.sh -d "$release" "$component" "$abort_on_fail" "app02" "$transfer_flag" >> $workspace/logs/Deployment/"${logfile}"
    ...
    then
        ...
        then
            ...
            #sshpass -p "not4dev!" ssh -o "StrictHostKeyChecking=no" root@$env "bash -s" -- < $workspace/deploy_on_server.sh -d "$release" "$component" "$abort_on_fail" "app01" > $workspace/logs/Deployment/"${logfile}"
            #sshpass -p "not4dev!" ssh -o "StrictHostKeyChecking=no" root@$env 'ssh app02' "bash -s" -- < $workspace/deploy_on_server.sh -d "$release" "$component" "$abort_on_fail" "app02" "$transfer_flag" >> $workspace/logs/Deployment/"${logfile}"
        ...
        then
            ...
            do
                ...
                then
                    ...
                    sshpass -p "Carnival@123" ssh -o "StrictHostKeyChecking=no" root@$ipaddr "bash -s" -- < $workspace/deploy_on_server.sh -t "$release" "$component" "$abort_on_fail" "app01" "$transfer_flag"> $workspace/logs/Deployment/"${logfile}"
                    ...
                    sshpass -p "not4dev!" ssh -o "StrictHostKeyChecking=no" root@$ipaddr "bash -s" -- < $workspace/deploy_on_server.sh -t "$release" "$component" "$abort_on_fail" "app01" "$transfer_flag" > $workspace/logs/Deployment/"${logfile}"
                ...
                then
                    ...
                    chksum_status=`grep "md5sum is not matching" $workspace/logs/Deployment/${logfile} | wc -l`
    ...
    then
        if [ "$env" == "192.168.248.161" ]
        then
            sshpass -p "Carnival@123" ssh root@$env "bash -s" -- < $workspace/deploy_on_server.sh -r "$release" "$component" "$abort_on_fail" "app01">> $workspace/logs/Deployment/"${logfile}"
            sshpass -p "Carnival@123" ssh root@$env 'ssh app02' "bash -s" -- < $workspace/deploy_on_server.sh -r "$release" "$component" "$abort_on_fail" "app02" "$transfer_flag" >> $workspace/logs/Deployment/"${logfile}"
            ...
            :
            #sshpass -p "not4dev!" ssh root@$env "bash -s" -- < $workspace/deploy_on_server.sh -r "$release" "$component" "$abort_on_fail" "app01"> $workspace/logs/Deployment/"${logfile}"
            #sshpass -p "not4dev!" ssh root@$env 'ssh app02' "bash -s" -- < $workspace/deploy_on_server.sh -r "$release" "$component" "$abort_on_fail" "app02" "$transfer_flag" >> $workspace/logs/Deployment/"${logfile}"
    ...
    if [ -f $workspace/logs/Deployment/${logfile} ] && ([ "$action" == "Deploy" ] || [ "$action" == "Rollback" ])
    then
        sed -n '/STATUS( app01 )/,/Checking if components/p' $workspace/logs/Deployment/${logfile} > $workspace/logs/Deployment/email_body.txt
        sed -n '/STATUS( app02 )/,$p' $workspace/logs/Deployment/${logfile} >> $workspace/logs/Deployment/email_body.txt
        sed -i '/Transferring artifacts to app02/d' $workspace/logs/Deployment/email_body.txt
        sed -i '/UTC 20/d' $workspace/logs/Deployment/email_body.txt
        sed -i '/Checking if components are present/d' $workspace/logs/Deployment/email_body.txt
    fi
    ...
    log "An exception occured in making ssh connections."
    exit 1
