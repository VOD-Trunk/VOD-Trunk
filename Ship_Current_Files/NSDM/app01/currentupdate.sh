...
APIKEY=`curl -i -H "Content-Type: application/x-www-form-urlencoded" \
  ...
  "http://admin.iptv.nsdmdomain.hal.com/v2/private/auth?debug" 2>/dev/null |\
  awk -F\" '/apikey/ {print $4}'`

echo >>/root/weatherupdate/refreshcurrent.log
date >>/root/weatherupdate/refreshcurrent.log
/usr/bin/curl -s -H "Authorization:${APIKEY}" "http://admin.iptv.nsdmdomain.hal.com/location/private/refreshcurrent" >>/root/weatherupdate/refreshcurrent.log 2>&1

#echo >>/root/weatherupdate/refreshforecast.log
#date >>/root/weatherupdate/refreshforecast.log
#/usr/bin/curl -s -H "Authorization:${APIKEY}" "http://admin.iptv.nsdmdomain.hal.com/location/private/refreshforecast" >>/root/weatherupdate/refreshforecast.log 2>&1
