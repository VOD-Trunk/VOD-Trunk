node {
    ...
    if( "${Activity}" != "Promote" ){
        ...
        def user_type = jconf.jenkins.users."${UserName}".user_type
       ...
       stage('deploy'){
        ...
        stage('Promote'){
           ...
           sh """
                ...
                ${env.WORKSPACE}/checkArtifactProperty.sh "${Deployment_env}" "${Activity}" "${Release_version}" "${env.USERNAME}" "${env.PASSWORD}" "$user_type" "${Set_status_done_on}"
